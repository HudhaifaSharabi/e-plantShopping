                ...
                <div className="product-grid">
                    {plantsArray.map((category, index) => ( // Loop through each category in plantsArray
                        <div key={index}> {/* Unique key for each category div */}
                            <h1 > {/* Class for styling category title */}
                            ...
                            {category.plants.map((plant, plantIndex) => ( // Loop through each plant in the current category
                                ...
                                <button
                                    className={`product-button ${cart.find(item => item.name === plant.name) ? "added-to-cart" : ""}`} // Add a class based on whether the plant is added to cart
                                    onClick={() => handleAddToCart(plant)} // Handle adding plant to cart
                                    disabled={cart.find(item => item.name === plant.name)} // Disable button if plant is already in cart
                                    
                                >
                                    {cart.find(item => item.name === plant.name) ? 'Added to Cart' : 'Add to Cart'} {/* Change button text based on whether the plant is added to cart or not */}
                                </button>
                                </div>
                            ))}
                            </div>
